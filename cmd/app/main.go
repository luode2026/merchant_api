...
import (
	"fmt"
	"merchant_api/internal/app/router"
...
func main() {
	...
	// 设置 Gin 模式
	// gin.SetMode(cfg.Server.App.Mode)

	// 初始化路由
	r := router.SetupRouter()

	// 启动服务
	addr := fmt.Sprintf(":%d", cfg.Server.App.Port)
	logger.Info(fmt.Sprintf("App 服务启动在端口 %d", cfg.Server.App.Port))
